...
GO_up=TG(up)
GO_down=TG(down)
write.table(GO_up,"fruit_GO_results/fruit_up.go",sep="\t",col.names=T,row.names=F,quote=F)
write.table(GO_down,"fruit_GO_results/fruit_down.go",sep="\t",col.names=T,row.names=F,quote=F)
system("perl ~/zihailing/RNA_seq_script/Topgo_buqi.pl /psc/home/zihailing/zihailing/species_20151012/Ath/GO.terms_and_ids fruit_GO_results/fruit_up.go > fruit_GO_results/fruit_up.go.tmp ")
system("mv fruit_GO_results/fruit_up.go.tmp fruit_GO_results/fruit_up.go")
system("perl ~/zihailing/RNA_seq_script/Topgo_buqi.pl /psc/home/zihailing/zihailing/species_20151012/Ath/GO.terms_and_ids fruit_GO_results/fruit_down.go > fruit_GO_results/fruit_down.go.tmp ")
system("mv fruit_GO_results/fruit_down.go.tmp fruit_GO_results/fruit_down.go")

pdf("fruit_GO_results/fruit_up_go.pdf")
GR=read.table("fruit_GO_results/fruit_up.go",header=T,sep="\t",quote="")
GS=subset(GR,GR$pvalue<0.001)
write.table(GS,"fruit_GO_results/fruit_up.go.sig",sep="\t",col.names=T,row.names=F,quote=F)
...
pdf("fruit_GO_results/fruit_down_go.pdf")
GR=read.table("fruit_GO_results/fruit_down.go",header=T,sep="\t",quote="")
GS=subset(GR,GR$pvalue<0.001)
write.table(GS,"fruit_GO_results/fruit_down.go.sig",sep="\t",col.names=T,row.names=F,quote=F)
PV=as.numeric(GS$pvalue)
names(PV)= GS$Term
func=GS$catagory
GS$catagory=as.vector(GS$catagory)
