...
############################################################################################################
de.out=exactTest(cds,pair=c("VeinWT","Vein5_2"))
...
write.table(all,"results34/Vein.edgeR.out",sep="\t",row.names=T,col.names=F,quote=F)

de.up=subset(all,((all$FDR<0.05) & (all$logFC) >=1))
de.down=subset(all,((all$FDR<0.05) & (all$logFC) <= -1))
write.table(de.up,"results34/Vein.up",sep="\t",row.names=T,col.names=F,quote=F)
write.table(de.down,"results34/Vein.down",sep="\t",row.names=T,col.names=F,quote=F)
...
write.table(de.log,"results34/Vein_Cutoff.log",sep="\t",row.names=F,col.names=F,quote=F)
